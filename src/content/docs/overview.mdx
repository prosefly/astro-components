...
<CardGrid>
  <Card icon="lucide:message-square-warning" title="Warn or emphasize" href="/components/callout/">
    Use `Callout` for notes, tips, warnings, and destructive guidance that
    should stand apart from the surrounding prose.
  </Card>
  <Card icon="lucide:panel-top" title="Show equivalent choices" href="/components/tabs/">
    Use `Tabs` for package managers, frameworks, languages, or other parallel
    examples where the reader should pick one.
  </Card>
  <Card icon="lucide:list-collapse" title="Keep secondary detail nearby" href="/components/accordion/">
    Use `Accordions` for optional explanations, FAQs, and background details
    that should not interrupt the main path.
  </Card>
  <Card icon="lucide:folder-tree" title="Explain project structure" href="/components/file-tree/">
    Use `FileTree` when a reader needs to create, find, or understand files and
    directories.
  </Card>
  <Card icon="lucide:route" title="Point readers forward" href="/components/cards/">
    Use `Card` and `CardGrid` for related guides, feature summaries, and
    next-step links.
  </Card>
  <Card icon="lucide:list-checks" title="Walk through a process" href="/components/steps/">
    Use `Steps` for installation, migration, and configuration procedures.
  </Card>
  <Card icon="lucide:terminal-square" title="Generate install tabs" href="/package-manager-tabs/">
    Use package-manager tabs when readers should choose between `pnpm`, `npm`,
    `yarn`, and `bun`.
  </Card>
  <Card icon="lucide:git-branch" title="Explain a system" href="/mermaid/">
    Use Mermaid code blocks for flows, sequences, states, classes, data models,
    and XY charts that should build into static SVG.
  </Card>
  <Card icon="lucide:image" title="Render image galleries" href="/images/">
  ...
  <Card icon="lucide:code-xml" title="Enhance code blocks" href="/expressive-code/">
    Add file icons and language labels to Expressive Code frames with the
    independent plugin.
  </Card>
</CardGrid>
...
3. **Use transforms for repeated patterns.**
   ...
   transform that matches the content.
    Dahlia and [Lotus](https://astro-theme-lotus.prosefly.dev/) enable both
    automatically.
...
Users of [Dahlia](https://astro-theme-dahlia.prosefly.dev/) or
[Lotus](https://astro-theme-lotus.prosefly.dev/) usually start at step 2 because
their themes already configure the Markdown transforms, Iconify preload
