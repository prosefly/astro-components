...
The `proseflyComponents()` integration enables callout directives so authors can write
callouts, also known as admonitions, without importing the component in every
MDX file.
...
Callout directives are enabled by default. Set
`markdown.calloutDirectives` to `false` when a project should keep directive
syntax untransformed.

```ts title="astro.config.ts"
proseflyComponents({
  markdown: { calloutDirectives: false },
})
```
