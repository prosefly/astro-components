...
The default Lucide icon set distinguishes code, configuration, style, image,
text, and generic files with both shape and color. Use
`iconSet="vscode-icons"` when a tree benefits from language- and tool-specific
icons. Folders keep the default Lucide icons, while file icons use the VS Code
icon set. Astro and MDX icons switch between light and dark variants with the
active theme.
