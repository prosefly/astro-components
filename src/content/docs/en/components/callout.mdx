---
title: Callout
description: Highlight information readers should not miss.
sidebar:
  order: 1
---

import { Callout } from '@prosefly/astro-components';

Use `Callout` when a reader needs to notice something before continuing:
requirements, safe defaults, limits, destructive actions, or shortcuts. Keep
callouts short enough to scan without breaking the page flow.

## Import

```mdx
import { Callout } from '@prosefly/astro-components';
```

## Write A Useful Callout

<Callout type="note" title="Document the default behavior">
  Notes are a safe default for implementation details, caveats, and extra
  context.
</Callout>

```mdx
<Callout type="note" title="Document the default behavior">
  Notes are a safe default for implementation details, caveats, and extra
  context.
</Callout>
```

## Choose The Right Type

<Callout type="note" title="Note">
  Use `note` for neutral context, defaults, and implementation details.
</Callout>

<Callout type="tip" title="Tip">
  Use `tip` for recommended workflows, shortcuts, and better ways to complete
  the same task.
</Callout>

<Callout type="warning" title="Warning">
  Use `warning` when a choice can cause broken output, confusing behavior, or
  extra setup work.
</Callout>

<Callout type="danger" title="Danger">
  Use `danger` for destructive commands, security-sensitive steps, and
  irreversible changes.
</Callout>

```mdx
<Callout type="note" title="Note">
  Use `note` for neutral context, defaults, and implementation details.
</Callout>

<Callout type="tip" title="Tip">
  Use `tip` for recommended workflows, shortcuts, and better ways to complete
  the same task.
</Callout>

<Callout type="warning" title="Warning">
  Use `warning` when a choice can cause broken output, confusing behavior, or
  extra setup work.
</Callout>

<Callout type="danger" title="Danger">
  Use `danger` for destructive commands, security-sensitive steps, and
  irreversible changes.
</Callout>
```

## With Rich Content

<Callout type="tip" title="Put the action first">
  Give the reader the command or decision before the explanation.

  ```sh
  pnpm add @prosefly/astro-components
  ```

  Add caveats only after the action is clear.
</Callout>

````mdx
<Callout type="tip" title="Put the action first">
  Give the reader the command or decision before the explanation.

  ```sh
  pnpm add @prosefly/astro-components
  ```

  Add caveats only after the action is clear.
</Callout>
````

## Props

| Prop | Type | Default |
| --- | --- | --- |
| `title` | `string` | none |
| `type` | `'note' \| 'tip' \| 'warning' \| 'danger'` | `'note'` |

## CSS Variables

Each callout type uses fixed semantic defaults. Override the type-specific
tokens when a site needs a different information, success, warning, or danger
palette.

| Token | Used For |
| --- | --- |
| `--pl-callout-note-color` | Note border mixing and fallback background |
| `--pl-callout-note-ink` | Note icon and title |
| `--pl-callout-note-bg` | Note background |
| `--pl-callout-tip-color` | Tip border mixing and fallback background |
| `--pl-callout-tip-ink` | Tip icon and title |
| `--pl-callout-tip-bg` | Tip background |
| `--pl-callout-warning-color` | Warning border mixing and fallback background |
| `--pl-callout-warning-ink` | Warning icon and title |
| `--pl-callout-warning-bg` | Warning background |
| `--pl-callout-danger-color` | Danger border mixing and fallback background |
| `--pl-callout-danger-ink` | Danger icon and title |
| `--pl-callout-danger-bg` | Danger background |
