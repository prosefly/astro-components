...
<Callout type="tip" title="Using prosefly lotus">
  Lotus enables the image gallery transform automatically. If your site uses
  `@prosefly/astro-theme-lotus`, the Markdown transform is already active.
</Callout>
...
If you are not using Lotus, add the rehype plugin in `astro.config.ts`.
...
<Callout type="warning" title="Avoid double configuration">
  Lotus already configures `rehypeImageGallery` and loads the theme styles. Add
  the plugin and runtime manually only in standalone Astro projects.
</Callout>
