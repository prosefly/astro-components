...
The `components()` integration enables image galleries so authors can write
...
<Callout type="tip" title="Using a Prosefly theme">
  ...
  `components()` integration. The transform, styles, and runtime are active
  automatically.
</Callout>
...
<Callout type="warning" title="Avoid double configuration">
  ...
  `components()` instance or manually load the gallery assets in theme
  projects.
</Callout>
