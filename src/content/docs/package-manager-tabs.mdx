...
<Callout type="tip" title="Using a Prosefly theme">
  ...
  `proseflyComponents()` integration. Theme projects do not need to add it again.
</Callout>
...
do not need to import those components manually. Supported commands are also
transformed when nested in components such as `Steps`; commands inside manual
`Tabs` and `TabItem` components are left unchanged to avoid nested tabs.
